...
/**
 * Unit test that, for every game, and for all combinations of options,
 * tests whether we can correctly run playouts in parallel.
 * 
 * @author Dennis Soemers
 */
public class TestParallelPlayoutsWithOptions
{
	...
	public void test()
	{
		...
		for (final File fileEntry : entries)
		{
			if (fileEntry.getName().contains(".lud"))
			{
				...
				final Game gameNoOptions = GameLoader.loadGameFromFile(fileEntry);
				...
				{
					System.out.println("Parallel Playouts test with options: " + optionCombination);
					...
					{
						contexts[i] = new Context(gameWithOptions, new Trial(gameWithOptions));
					...
					for (int parallelPlayout = 0; parallelPlayout < NUM_PARALLEL; ++parallelPlayout)
					{
						...
						while (moveIdx < loadedMoves.size())
						{
							...
							final Moves legalMoves = gameWithOptions.moves(context);
							final List<Action> loadedMoveAllActions = loadedMoves.get(moveIdx).getActionsWithConsequences(context);
							
							if (gameWithOptions.mode().mode() == ModeType.Alternating)
							{
								...
								gameWithOptions.apply(context, matchingMove);
							}
							else
							{
								...
								final int numPlayers = gameWithOptions.players().count();
								...
								gameWithOptions.apply(context, loadedMoves.get(moveIdx));
							}
